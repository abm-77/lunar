...
// assets are identified by the FNV-1a 64-bit hash of their basename (e.g.,
// "TriShader.umsh"). at init time the entire .umpack is loaded into memory and
// a flat table is built. rt_asset_load returns a pointer directly into that
// buffer (uncompressed entries only). compressed entries are a TODO: they print
// a warning and return an empty slice.
...
// rt_assets_init — passed as (ptr, len) from Umbral's []u8 slice calling
// convention.
void rt_assets_init(const uint8_t *path_ptr, uint64_t path_len) {
  ...
  if (magic != UMPACK_MAGIC) {
    goto bad_format;
  }
  if (endian != UMPACK_ENDIAN_LE) {
    goto bad_format;
  }
  ...
  for (uint32_t i = 0; i < entry_count; ++i) {
    ...
    memcpy(&data_offset, s_pack_data + cur, 8);
    cur += 8;
    memcpy(&compressed_len, s_pack_data + cur, 4);
    cur += 4;
    memcpy(&original_len, s_pack_data + cur, 4);
    cur += 4;
    ...
    if (compressed_len != original_len) {
      ...
      fprintf(stderr,
              "rt_assets_init: compressed asset '%.*s' not yet supported; "
              "skipping\n",
              (int)name_len, name);
      continue;
    }
...
void rt_asset_release(uint64_t id) {
  // no-op in v0: all assets are borrowed from s_pack_data for the process
  // lifetime
  (void)id;
}
