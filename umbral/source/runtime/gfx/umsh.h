...
// STAGES section layout (starts immediately after the section descriptor offset):
//   u32 stage_count
...
static inline bool umsh_find_section(const uint8_t *blob, uint64_t blob_len,
  ...
  const umsh_section_t *secs = (const umsh_section_t *)(blob + sizeof(umsh_header_t));
...
static inline bool umsh_find_stage(umsh_section_view_t sec, uint8_t stage_kind,
  ...
  p += 4;
  if (sec.size < 4 + (uint64_t)stage_count * sizeof(umsh_stage_record_t)) return false;
