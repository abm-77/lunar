...
struct MultiModFixture : ::testing::Test {
  ...
  Result<std::vector<LoadedModule>> load(const std::string &entry_rel) {
...
TEST_F(SemaFixture, IntLiteralWidensToU64Param) {
  ...
  auto sr = sema("const f := fn (x: u64) -> u64 { return x; }"
...
TEST_F(SemaFixture, SliceFieldLen) {
  auto sr = sema("const f := fn (s: []u8) -> u64 { return s.len; }");
  EXPECT_TRUE(sr.has_value()) << (sr.has_value() ? "" : sr.error().msg);
}

TEST_F(SemaFixture, SliceFieldPtr) {
  auto sr = sema("const f := fn (s: []u8) -> &u8 { return s.ptr; }");
  EXPECT_TRUE(sr.has_value()) << (sr.has_value() ? "" : sr.error().msg);
}
...
TEST_F(SemaFixture, EnumVariantType) {
  auto sr = sema("const Color: type = enum { Red, Green, Blue }"
...
TEST_F(SemaFixture, BreakOutsideLoopError) {
  auto sr = sema("const f := fn () -> void { break; }");
  EXPECT_FALSE(sr.has_value());
  if (!sr.has_value())
...
TEST_F(SemaFixture, ContinueOutsideLoopError) {
  auto sr = sema("const f := fn () -> void { continue; }");
  EXPECT_FALSE(sr.has_value());
  if (!sr.has_value())
    EXPECT_NE(sr.error().msg.find("outside"), std::string::npos)
        << "error was: " << sr.error().msg;
...
TEST_F(SemaFixture, GenericFreeFunction) {
  auto sr = sema("const identity<T: type>(x: T) -> T { return x; }"
...
TEST_F(SemaFixture, GenericStructStaticMethod) {
  auto sr = sema("const Pair<A: type, B: type> := struct { a: A, b: B }"
                 "impl Pair<A, B> {"
...
TEST_F(SemaFixture, GenericStructInstanceMethod) {
  // instance method on generic struct — return type must resolve through mono
  // subst
  auto sr = sema("const Pair<A: type, B: type> := struct { a: A, b: B }"
                 "impl Pair<A, B> {"
...
TEST_F(SemaFixture, MultipleInstantiationsOfSameGeneric) {
  ...
  auto sr = sema("const as_i32 := fn (x: i32) -> i32 { return x; }"
...
TEST_F(SemaFixture, ConstGenericParam) {
  auto sr = sema("const fixed_val<N: u32>() -> u32 { return N; }"
...
TEST_F(SemaFixture, NestedGenericStruct) {
  ...
  auto sr = sema("const Box<T: type> := struct { val: T }"
                 "const f := fn () -> i32 {"
...
TEST_F(MultiModFixture, CrossModuleGenericStructInit) {
  ...
  write("container.um", "@pub const Box<T: type> := struct { val: T }");
  write("main.um", "import container;"
                   "const f := fn () -> void {"
...
TEST_F(MultiModFixture, CrossModuleGenericStaticMethod) {
  ...
  write("pair.um", "@pub const Pair<A: type, B: type> := struct { a: A, b: B }"
                   "impl Pair<A, B> {"
                   "  const make := fn (a: A, b: B) -> Pair<A, B> {"
                   "    return Pair<A, B> { a = a, b = b };"
                   "  }"
                   "}");
  write("main.um", "import pair;"
                   "const f := fn () -> void {"
