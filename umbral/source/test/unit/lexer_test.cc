...
TEST(KeywordTable, AllKeywordsRecognised) {
  ...
  Case cases[] = {
      {"fn", TokenKind::KwFn},         {"const", TokenKind::KwConst},
      ...
      {"type", TokenKind::KwType},     {"impl", TokenKind::KwImpl},
      {"import", TokenKind::KwImport}, {"true", TokenKind::KwTrue_},
      {"false", TokenKind::KwFalse_},  {"var", TokenKind::KwVar},
...
TEST_F(LexFixture, SingleCharPunctuation) {
  ...
  Case cases[] = {
      {"(", TK::LParen},    {")", TK::RParen},   {"{", TK::LBrace},
      {"}", TK::RBrace},    {"[", TK::LBracket}, {"]", TK::RBracket},
      {",", TK::Comma},     {".", TK::Dot},      {";", TK::Semicolon},
      {"+", TK::Plus},      {"-", TK::Minus},    {"*", TK::Star},
      {"/", TK::Slash},     {"=", TK::Equal},    {"!", TK::Bang},
      {"<", TK::Less},      {">", TK::Greater},  {":", TK::Colon},
      {"&", TK::Ampersand},
  };
  for (auto &c : cases) {
    auto k = kinds(c.src);
...
TEST_F(LexFixture, TwoCharOperators) {
  ...
  Case cases[] = {
      {"==", TK::EqualEqual},   {"!=", TK::BangEqual},  {"<=", TK::LessEqual},
      {">=", TK::GreaterEqual}, {"+=", TK::PlusEqual},  {"-=", TK::MinusEqual},
      {"*=", TK::StarEqual},    {"/=", TK::SlashEqual}, {"->", TK::Arrow},
      {"::", TK::ColonColon},   {":=", TK::ColonEqual},
  };
  for (auto &c : cases) {
    auto k = kinds(c.src);
    ASSERT_EQ(k.size(), 1u) << "src: " << c.src;
...
TEST_F(LexFixture, FunctionSignature) {
  ...
  std::vector<TK> expected = {
      ...
      TK::KwFn,  TK::LParen, TK::Ident,   TK::Colon,  TK::Ident,
      TK::Comma, TK::Ident,  TK::Colon,   TK::Ident,  TK::RParen,
      TK::Arrow, TK::Ident,  TK::LBrace,  TK::RBrace,
  };
  EXPECT_EQ(k, expected);
}
...
TEST_F(LexFixture, StructDefinition) {
  ...
  std::vector<TK> expected = {
      TK::KwType, TK::Ident, TK::Equal, TK::KwStruct, TK::LBrace,
      TK::Ident,  TK::Colon, TK::Ident, TK::Comma,    TK::RBrace,
  };
  EXPECT_EQ(k, expected);
}
