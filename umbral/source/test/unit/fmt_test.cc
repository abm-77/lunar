...
TEST(FmtTest, PlainText) { EXPECT_EQ(capture("hello", {}), "hello\n"); }

TEST(FmtTest, EmptyString) { EXPECT_EQ(capture("", {}), "\n"); }
...
TEST(FmtTest, I64Large) {
  EXPECT_EQ(
      capture("{}", {{ARG_I64, static_cast<uint64_t>(1000000000LL * 9), 0}}),
      "9000000000\n");
}

TEST(FmtTest, U32) { EXPECT_EQ(capture("{}", {{ARG_U32, 255, 0}}), "255\n"); }
...
TEST(FmtTest, EscapedOpenBrace) { EXPECT_EQ(capture("{{", {}), "{\n"); }

TEST(FmtTest, EscapedCloseBrace) { EXPECT_EQ(capture("}}", {}), "}\n"); }
