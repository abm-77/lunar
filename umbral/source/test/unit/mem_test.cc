...
TEST_F(MemTest, SliceFromAllocWriteAndRead) {
  ...
  um_alloc_handle_t h =
      rt_alloc(count * sizeof(int32_t), alignof(int32_t), 0, 0);
  ASSERT_NE(h, 0u);

  um_slice_u8_t s =
      rt_slice_from_alloc(h, sizeof(int32_t), alignof(int32_t), count, 0, 0);
  ASSERT_NE(s.ptr, nullptr);
  EXPECT_EQ(s.len, count * sizeof(int32_t));

  // Write through the slice pointer.
  int32_t *arr =
      const_cast<int32_t *>(reinterpret_cast<const int32_t *>(s.ptr));
...
TEST_F(MemTest, FreeNullHandleAborts) { EXPECT_DEATH(rt_free(0, 0), ""); }
