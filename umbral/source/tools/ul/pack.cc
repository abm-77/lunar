...
static bool read_file_bin(const char *path, std::vector<uint8_t> &out) {
...
int shader_pack(const char *vert_spv_path, const char *frag_spv_path,
  ...
  if (!read_file_bin(vert_spv_path, vs)) return -1;
  if (!read_file_bin(frag_spv_path, fs)) return -1;
  if (!read_file_bin(umrf_path, umrf)) return -1;
