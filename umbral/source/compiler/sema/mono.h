...
// one pending mono instantiation queued for body-checking by the sema
// orchestrator.
struct PendingMono {
  SymbolId mono_id;
  u32 generic_mod_idx;
...
inline void flush_int_defaults(
  ...
  for (auto &[tv, _] : int_default_vars)
    if (unifier.resolve(IType::fresh(tv)).is_var) unifier.bindings[tv] = i32_t;
}
...
inline std::vector<CTypeId>
infer_type_args(const Symbol &gsym, const Module &g_mod,
                const TypeLowerer &g_lowerer, Unifier &unifier,
...
private:
  SymbolId prepare(SymbolId generic_id, const std::vector<CTypeId> &type_args,
    ...
    mi->const_values = const_values;
    {
      ...
      // detect &self first param: store separately so concrete_params is 1-to-1
      // with call args
      u32 params_start = 0;
    ...
    pending.push_back({mono_id, gsym_mod, static_cast<u32>(gsym.generics_start),
