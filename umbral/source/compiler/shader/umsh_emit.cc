...
bool emit_umsh(std::string_view out_dir, std::string_view shader_name,
  ...
  for (const auto &s : stages) {
    ...
    // each payload is 4-byte aligned (SPIR-V is always 4-byte-aligned by
    // definition)
  ...
  uint64_t header_size =
      24 + static_cast<uint64_t>(n_sections) * 24; // header + section table

  uint64_t stages_offset = align_up(header_size, 8);
  uint64_t refl_offset =
      refl ? align_up(stages_offset + stages_data_size, 8) : 0;
  ...
  std::string path =
      std::string(out_dir) + "/" + std::string(shader_name) + ".umsh";
