// lower shader stage/helper BodyIR to um.shader + arith + scf + memref MLIR ops.
// no SPIR-V is emitted here — that happens in shader_spirv_lower.cc.
...
mlir::Value ShaderEmitCtx::emit(mlir::OpBuilder &b, NodeId nid) {
  ...
  case NodeKind::IntLit:
    return b.create<mlir::arith::ConstantOp>(loc,
        b.getIntegerAttr(b.getI32Type(), static_cast<int64_t>(na))).getResult();

  case NodeKind::FloatLit: {
    double dval = ir.float_lits[na];
    return b.create<mlir::arith::ConstantOp>(loc,
        b.getFloatAttr(b.getF32Type(), static_cast<float>(dval))).getResult();
  }

  case NodeKind::BoolLit:
    return b.create<mlir::arith::ConstantOp>(loc,
        b.getBoolAttr(na != 0)).getResult();
  ...
  case NodeKind::Field: {
    ...
    if (detect_io_access(nid, field_str, sub_str)) {
      ...
      return b.create<LoadInputOp>(loc, ty,
          b.getStringAttr(field_str), b.getStringAttr(sub_str)).getResult();
    }
    ...
    if (vec_ty) {
      ...
      if (idx >= 0)
        return b.create<mlir::vector::ExtractOp>(loc, base_val,
            static_cast<int64_t>(idx)).getResult();
    }
    // draw_packet field access → DrawPacketFieldOp (result is i32)
    if (mlir::isa<DrawPacketType>(base_val.getType())) {
      auto field_name = sym_name(nb);
      return b.create<DrawPacketFieldOp>(loc, b.getI32Type(), base_val,
          b.getStringAttr(field_name)).getResult();
    }
    return {};
  }
  ...
  case NodeKind::Index: {
    ...
    if (vec_ty)
      return b.create<mlir::vector::ExtractOp>(loc, base,
          mlir::OpFoldResult(idx)).getResult();
    return {};
  }
  ...
  case NodeKind::Unary: {
    ...
    if (op == TokenKind::Minus) {
      ...
      auto zero = b.create<mlir::arith::ConstantOp>(loc,
          b.getIntegerAttr(child.getType(), 0)).getResult();
      return b.create<mlir::arith::SubIOp>(loc, zero, child).getResult();
    }
    if (op == TokenKind::Bang)
      return b.create<mlir::arith::XOrIOp>(loc, child,
          b.create<mlir::arith::ConstantOp>(loc, b.getBoolAttr(true)).getResult()
          ).getResult();
    return {};
  }
  ...
  case NodeKind::Binary: {
    ...
    bool is_float = mlir::isa<mlir::FloatType>(lhs.getType()) ||
                    (mlir::isa<mlir::VectorType>(lhs.getType()) &&
                     mlir::isa<mlir::FloatType>(mlir::cast<mlir::VectorType>(
                         lhs.getType()).getElementType()));
    switch (op) {
    case TokenKind::Plus:
      return (is_float ? (mlir::Value)b.create<mlir::arith::AddFOp>(loc, lhs, rhs)
                       : (mlir::Value)b.create<mlir::arith::AddIOp>(loc, lhs, rhs));
    case TokenKind::Minus:
      return (is_float ? (mlir::Value)b.create<mlir::arith::SubFOp>(loc, lhs, rhs)
                       : (mlir::Value)b.create<mlir::arith::SubIOp>(loc, lhs, rhs));
    case TokenKind::Star:
      return (is_float ? (mlir::Value)b.create<mlir::arith::MulFOp>(loc, lhs, rhs)
                       : (mlir::Value)b.create<mlir::arith::MulIOp>(loc, lhs, rhs));
    case TokenKind::Slash:
      return (is_float ? (mlir::Value)b.create<mlir::arith::DivFOp>(loc, lhs, rhs)
                       : (mlir::Value)b.create<mlir::arith::DivSIOp>(loc, lhs, rhs));
    case TokenKind::EqualEqual:
      return (is_float
          ? (mlir::Value)b.create<mlir::arith::CmpFOp>(loc,
              mlir::arith::CmpFPredicate::OEQ, lhs, rhs)
          : (mlir::Value)b.create<mlir::arith::CmpIOp>(loc,
  ...
  case NodeKind::Call: {
    ...
    if (!cname.empty()) {
      if (cname == "vec2" || cname == "vec3" || cname == "vec4") {
        ...
        mlir::Value vec = b.create<mlir::arith::ConstantOp>(loc,
            mlir::DenseElementsAttr::get(vec_ty,
                llvm::ArrayRef<float>(zeros))).getResult();
        for (u32 k = 0; k < n && k < args_count; ++k) {
          ...
          vec = b.create<mlir::vector::InsertOp>(loc, elem, vec,
              static_cast<int64_t>(k)).getResult();
        }
        return vec;
      }
    }
  ...
  case NodeKind::Block: {
    ...
    for (u32 k = 0; k < stmt_count; ++k)
      emit(b, ir.nodes.list[stmt_start + k]);
    return {};
  }
  ...
  case NodeKind::AssignStmt: {
    ...
    case TokenKind::PlusEqual:
      result = is_float ? (mlir::Value)b.create<mlir::arith::AddFOp>(loc, cur, rhs)
                        : (mlir::Value)b.create<mlir::arith::AddIOp>(loc, cur, rhs);
      break;
    case TokenKind::MinusEqual:
      result = is_float ? (mlir::Value)b.create<mlir::arith::SubFOp>(loc, cur, rhs)
                        : (mlir::Value)b.create<mlir::arith::SubIOp>(loc, cur, rhs);
      break;
    case TokenKind::StarEqual:
      result = is_float ? (mlir::Value)b.create<mlir::arith::MulFOp>(loc, cur, rhs)
                        : (mlir::Value)b.create<mlir::arith::MulIOp>(loc, cur, rhs);
      break;
    case TokenKind::SlashEqual:
      result = is_float ? (mlir::Value)b.create<mlir::arith::DivFOp>(loc, cur, rhs)
  ...
  case NodeKind::IfStmt: {
    ...
    b.create<mlir::scf::IfOp>(loc, cond,
        [&](mlir::OpBuilder &nb2, mlir::Location nl) {
          emit(nb2, then_nid);
          nb2.create<mlir::scf::YieldOp>(nl);
        },
        has_else ? mlir::function_ref<void(mlir::OpBuilder &, mlir::Location)>(
            [&](mlir::OpBuilder &nb2, mlir::Location nl) {
              emit(nb2, else_nid);
              nb2.create<mlir::scf::YieldOp>(nl);
  ...
  case NodeKind::ForStmt: {
    ...
    b.create<mlir::scf::WhileOp>(loc, mlir::TypeRange{}, mlir::ValueRange{},
        [&](mlir::OpBuilder &nb2, mlir::Location nl, mlir::ValueRange) {
          ...
          mlir::Value cond_val;
          if (cond_nid)
            cond_val = emit(nb2, cond_nid);
          if (!cond_val)
            cond_val = nb2.create<mlir::arith::ConstantOp>(nl,
                nb2.getBoolAttr(true)).getResult();
  ...
  case NodeKind::ForRange: {
    ...
    if (auto vec_ty = mlir::dyn_cast<mlir::VectorType>(src_ty)) {
      ...
      len_val = b.create<mlir::arith::ConstantOp>(loc,
          b.getI32IntegerAttr(vec_ty.getNumElements())).getResult();
    } else {
  ...
  case NodeKind::ShaderFrameRead: {
    ...
    mlir::Type elem_ty = type_from_typeid(b, type_ast, interner, static_cast<TypeId>(nb));
    if (!elem_ty) return {};
    return b.create<FrameReadOp>(loc, elem_ty, offset,
        mlir::TypeAttr::get(elem_ty)).getResult();
  }
...
emit_func(mlir::OpBuilder &b, mlir::Location loc,
          mlir::ModuleOp mlir_mod, const LoadedModule &lm,
          const SemaResult &sema, const Interner &interner,
          u32 mod_idx, SymId shader_type, SymId type_sym_id_sym,
          SymbolId method_sym_id, std::string_view func_name,
  ...
  if (stage_attr)
    fn->setAttr("stage", b.getStringAttr(*stage_attr));
  ...
  ShaderEmitCtx ectx{
      *b.getContext(), loc, lm.ir, lm.type_ast, lm.mod,
      sema, interner, mod_idx,
      find_self_sym(msym, lm.mod, interner),
      shader_type,
      {}, {}, helper_fns, entry};
...
lower_to_mlir(mlir::MLIRContext &ctx,
              ...
              const SemaResult &sema,
              const Interner &interner) {
  ctx.loadDialect<UmShaderDialect,
  ...
  for (u32 mi = 0; mi < static_cast<u32>(modules.size()); ++mi) {
    ...
    std::unordered_map<SymId, std::unordered_map<SymId, mlir::func::FuncOp>> per_shader_helpers;
    ...
    for (const ShaderFnInfo &sfi : mod.shader_fns) {
      ...
      auto fn = emit_func(b, loc, mlir_mod, lm, sema, interner, mi,
                          sfi.shader_type, type_sid, method_sid, fn_name,
                          std::nullopt, helper_map);
    ...
    for (const ShaderStageInfo &si : mod.shader_stages) {
      ...
      emit_func(b, loc, mlir_mod, lm, sema, interner, mi,
                si.shader_type, type_sid, method_sid, fn_name,
                stage_str, helper_map);
    }
  }
