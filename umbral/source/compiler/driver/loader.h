...
inline Result<u32> ModuleLoader::load(const std::filesystem::path &abs_path) {
  ...
  if (in_progress.count(canonical_key)) {
    return std::unexpected(
        Error{.msg = "circular import detected: " + canonical_key});
  }
  ...
  if (!f)
    return std::unexpected(
        Error{.msg = "cannot open '" + abs_path.string() + "'"});
  ...
  if (!lex_r)
    return std::unexpected(
        Error{.msg = format_error(lex_r.error(), src, abs_path.string())});
  ...
  for (const ImportDecl &imp : parser.mod.imports) {
    ...
    try {
      ...
      return std::unexpected(
          Error{.msg = "module not found: '" + child_abs.string() + "'"});
    }
    ...
    if (!inserted && it->second != child_idx) {
      ...
      return std::unexpected(Error{
          .msg = "import alias '" + std::string(interner.view(alias_id)) +
